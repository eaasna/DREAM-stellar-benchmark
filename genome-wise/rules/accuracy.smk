...
rule blast_compare_valik:
	input:
		ref_meta = "/dev/shm/" + dream_out + "/b" + str(bin_list[0]) + "_l" + str(min_lens[0]) + "_cmin" + str(cmin_list[0]) + "_cmax" + str(cmax_list[0]) + ".bin",
		test_files = expand(blast_out + "/" + run_id + "_e{ev}_k{k}.bed", ev = evalues, k = blast_kmer_lengths),
		truth_files = expand(dream_out + "/b{b}_l{min_len}_cmin{cmin}_cmax{cmax}_e{er}_ent{bin_ent}_cap{max_cap}_carts{max_carts}_t{t}_rp{rp}_rl{rl}.gff", b = bin_list, min_len = min_lens, cmin = cmin_list, cmax = cmax_list, er = errors, bin_ent = bin_entropy_cutoffs, max_cap = cart_max_capacity, max_carts = max_queued_carts, t = search_threads, rp = repeat_periods, rl = repeat_lengths),
	output:
